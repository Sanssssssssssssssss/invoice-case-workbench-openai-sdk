...
class RoleRegistry:
    ...
    def call(self, role: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        trace_metadata = {
            ...
            "temperature": _temperature(model, self.llm.settings.llm_temperature, self.llm.settings.llm_thinking_type),
    ...
    def agent(self, role: str) -> Agent:
        ...
        return Agent(
            ...
            model_settings=ModelSettings(
                temperature=_temperature(model, self.llm.settings.llm_temperature, self.llm.settings.llm_thinking_type),
                extra_body=_model_extra_body(model, self.llm.settings.llm_thinking_type),
            ),
...
def _temperature(model_name: str, default: float, thinking_type: str | None) -> float:
