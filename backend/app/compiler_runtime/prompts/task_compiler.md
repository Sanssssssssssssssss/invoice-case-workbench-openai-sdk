...
- Every CHECK carrying a facet with required_semantic_roles must declare and test all of those roles. Keep interdependent observation, applicability, and reconciliation roles in one CHECK unless each resulting CHECK can independently satisfy the facet's complete semantic and minimum-proof contract. Mere component-amount presence never completes stated-component validity.
- Invalid role split: three CHECKs for one facet, each declaring only observation, applicability, or reconciliation. Valid grouping: one CHECK declares every required role and its statement jointly tests them; split only when every resulting CHECK independently declares and tests every required role.
