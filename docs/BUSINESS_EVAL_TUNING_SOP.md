...
| E011 | 0006 | After splitting a CHECK, TaskCompiler may leave a removed node alias in an ALL dependency and fail Plan validation. | Add one output-time referential-integrity self-check for roots, status dependencies, and upstream dataflow; do not auto-rewrite the Plan. | Split plans contain no unknown/stale node references and reach Executor without adding a repair loop. | Deterministic Compiler/Signature gate passed: 94/94. Fresh Eval pending. |
