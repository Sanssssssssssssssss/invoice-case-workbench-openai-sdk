...
| E007 | 0053 | A canonical partial report already exists, but `collecting_materials` prevents the deterministic file-write continuation and causes repeated Report Writer delegation. | Keep `ready_for_report` as the gate for starting Report Writer, but allow an existing canonical report to continue to `write_case_file` and PDF approval. | Partial cases persist the existing report without creating reports for unready cases or changing proof/reportability. | Deterministic tests pass: the new boundary test and all 60 TurnRunner tests are green. Fresh Eval pending. |
