...
- Reconstruct a final aggregate from independently established upstream values and compare it with the reported aggregate only at terminal reconciliation. In the final-aggregate CHECK statement, name the upstream aggregate as independently derived or recomputed; an unqualified label such as "subtotal" is ambiguous because it may mean the reported value. Do not feed a possibly wrong reported aggregate back into upstream component checks.
...
- Every CHECK carrying a facet with required_semantic_roles must declare all of those roles in semantic_role_refs. The number of CHECKs remains free: split by independently answerable component or source scope when useful, but do not create an amount-presence-only CHECK for stated-component validity. Mere component-amount presence covers only COMPONENT_OBSERVATION and cannot complete COMPONENT_APPLICABILITY or COMPONENT_RECONCILIATION.
