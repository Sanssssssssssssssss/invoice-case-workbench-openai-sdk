...
| E009 | 0025 / 0053 | A redundant business matrix repeats a root Requirement ID on leaf rows, so a valid local `SUPPORTED` leaf is misread as a conflicting root status. | Delete the duplicate matrix; show Requirement IDs only in the root table and canonical Check IDs in the leaf table. Keep status validation strict. | Mixed leaf statuses under a `NOT_FOUND` root persist without weakening root or leaf validation. | Keep: 64/64 deterministic tests; fresh 0053 E011 accepted mixed leaf statuses and generated MD, PDF, and final reply. Fresh 0025 E012 failed earlier at VAT Binding/final Witness and never reached Writer, so it is not a report-boundary regression. |
| E010 | 0006 | Plan conformance incorrectly requires every `stated_components` CHECK to carry every semantic role, so a valid split plan is rejected before execution. | Require the reachable CHECKs for one facet to cover the semantic-role set collectively; each CHECK declares only the role it tests. | Amount-only plans still fail, while observation/applicability/reconciliation may be separate atomic CHECKs without weakening Kernel closure. | Keep: deterministic gates passed (94 + 185 tests); fresh E014 produced the intended three-way split, then exposed a stale dependency alias. |
| E011 | 0006 | After splitting a CHECK, TaskCompiler may leave invalid graph references or put proof dataflow into CHECK `depends_on`. | Add one output-time referential-integrity self-check; do not auto-rewrite the Plan. | Split plans reach Executor without invalid references. | Rejected and removed: E015 still failed before Executor because CHECKs used both `depends_on` and `upstream_check_ids`. The existing prompt already states this boundary three times; more wording is not a root fix. Open gap: one bounded TaskCompiler conformance repair or a simpler Plan action contract. |
