...
def load_requirement_pack(path: Path = POLICY_PATH) -> dict[str, Any]:
    ...
    try:
        graph = {key: set(value.get("premise_requirements") or []) for key, value in requirements.items()}
...
if not DYNAMIC_SUPPORT_REQUIREMENTS.issubset(KNOWN_REQUIREMENTS - COMPILER_DERIVED_REQUIREMENTS):
